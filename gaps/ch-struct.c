...
/*
 * EXPERIMENT: the struct-spelled pool, faithfully - the retest for
 * CODEGENGAPS entry 19's family.  Static storage instead of calloc
 * only because entry 18 (malloc losing its size argument) is still
 * live; every other shape below is the original's.
 */
#define	POOLBUFS	16
static struct buf pool[POOLBUFS];
static struct buf *bufs = pool;
public int nbufs;
