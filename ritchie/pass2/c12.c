...
register union tree *tree;
{
	...
	if ((dope & LEAF) != 0) {
		if (op == FCON) {
			...
			if (fp11.iv[1] == 0 && fp11.iv[2] == 0 && fp11.iv[3] == 0) {
				tree->t.op = SFCON;
	...
	case ASAND:
		tree->t.op = ASANDN;
		tree->t.tr2 =
			tnode(COMPL, tree->t.tr2->t.type, tree->t.tr2, TNULL);
		break;
	...
	case MINUS:
		if ((t = isconstant(tree->t.tr2))
			&& (!uns(t) || tree->t.type != LONG)
			&& (t->t.type != INT || t->c.value != SIGNBIT)) {
			tree->t.op = PLUS;
			if (t->t.type == DOUBLE)
				/*
				 * Negate FP by flipping sign bit
				 */
				t->c.value ^= SIGNBIT;
			else
				t->c.value = -t->c.value;
...
again:
	...
	if ((dope & RELAT) != 0) {
		if ((d1 = degree(tree->t.tr1)) < (d2 = degree(tree->t.tr2))
			|| d1 == d2 && tree->t.tr1->t.op == NAME
			&& tree->t.tr2->t.op != NAME) {
			t = tree->t.tr1;
			tree->t.tr1 = tree->t.tr2;
			tree->t.tr2 = t;
	...
	case FSELA:
		...
		if (tree->t.tr2->t.op == CON
			&& tree->F.mask == tree->t.tr2->c.value) {
			tree->t.op = ASOR;
			return (optim(tree));
		}
	...
	case ASTIMES:
		if (tree->t.tr2->t.op == CON && tree->t.tr2->c.value == 1) {
			if (op == PTOI)
				return (optim
						(tnode
						 (LTOI, INT, paint(tree->t.tr1, LONG), TNULL)));
...
constant:
		if (tree->t.tr1->t.op == CON && tree->t.tr2->t.op == CON) {
			constfold(op, &tree->t.tr1->c.value, tree->t.tr2->c.value,
					  tree->t.type);
			return (tree->t.tr1);
		}
...
def:
	default:
		if (dope & RELAT) {
			if (tree->t.tr1->t.type == LONG || tree->t.tr1->t.type == UNLONG) /* long 
																			   * relations 
																			   * are 
																			   * a 
																			   * mess 
																			   */
				d1 = 10;
			if (opdope[tree->t.tr1->t.op] & RELAT
		...
		tree->t.degree =
			d1 == d2 ? d1 + islong(tree->t.type) : MAX(d1, d2);
		break;
	}
	return (tree);
}
...
again:
	...
	case ITOL:
		if (subtre->t.op == CON && subtre->t.type == INT
			&& subtre->c.value < 0) {
	...
	case LTOI:
		...
		case EXOR:
			subtre->t.tr2 =
				tnode(LTOI, tree->t.type, subtre->t.tr2, TNULL);
		case NEG:
		case COMPL:
			subtre->t.tr1 =
				tnode(LTOI, tree->t.type, subtre->t.tr1, TNULL);
			subtre->t.type = tree->t.type;
			return (optim(subtre));
		}
		break;
	...
	case FSELR:
		...
		tree->t.tr1->t.tr1->c.value =
			(1 << tree->t.tr1->t.tr1->c.value) - 1;
		return (optim(tree));
	...
	case STAR:
		...
		if ((subtre->t.op == INCAFT || subtre->t.op == DECBEF)
			...
			&& p->t.op == NAME && p->n.class == REG
			&& p->t.type == subtre->t.type) {
	...
	case COMPL:
		...
		if (subtre->t.op == ITOL) {
			if (subtre->t.tr1->t.op == CON) {
				...
				if (uns(subtre->t.tr1))
					tree->l.lvalue =
						~(long) (unsigned) subtre->t.tr1->c.value;
				else
	...
	case NEG:
		...
		/*
		 * FP negation: flip sign bit
		 */
		if (subtre->t.op == SFCON) {
			subtre->c.value ^= SIGNBIT;
...
register union tree *tree;
{
	...
	if (!flt) {
		...
		for (i = acl.nextl; i > 0; i--) {
			d = t2[-1]->t.type == UNSIGN
				|| t2[0]->t.type == UNSIGN ? UNSIGN : INT;
...
ins:
	...
	if (!isfloat(tree)) {
		...
		if ((tree->t.op == TIMES || tree->t.op == LSHIFT)
			&& tree->t.tr2->t.op == CON && tree->t.tr2->c.value > 0
			&& tree->t.tr1->t.op == PLUS
			&& tree->t.tr1->t.tr2->t.op == CON) {
