...
* **Shadowing an outer name from a nested block.**  The declaration is
  renamed to `L<n>` but the references are not, so the inner variable
  is the outer one.  `rt_block.c` documents it and does not test it.
