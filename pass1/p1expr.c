...
uint8_t inStructTag = 0;        /* 8f85 - true when parsing struct/union tag */
...
/**************************************************
 * 17: 07F5 PMO +++
 * parseExprMode - Parse expression with specified mode
 **************************************************/
...
/**************************************************
 * 18: 0817 PMO +++
 * parseFuncArgs - Parse function call arguments
 **************************************************/
expr_t *parseFuncArgs(register attr_t *pa) {
    int16_t argIdx;
    int16_t argCnt;
    expr_t *argExpr;
    uint8_t tok;
    ...
    if ((argCnt != 1 || argIdx != 0 || !isVarOfType(pa, DT_VOID)) && argCnt && pa->dataType != DT_VARGS)
...
/**************************************************
 * 21: 0B93 PMO +++
 * isStaticAddr - Check if expression is a static address
 **************************************************/
bool isStaticAddr(register expr_t *st) {
    uint8_t op;
            ...
            case T_ID: /* c9a */
                ...
                if (!hasMember) {
                    if (st->sclass == 0) {
                        ...
                        else if (st->sclass == T_EXTERN && !(symFlags & S_EMITTED))
                            emitVar(st);
                    }
                    markReferenced(st);
                } /* d57 */
...
/**************************************************
 * 23: 10A8 PMO +++
 * reduceOp - Process operator from the stack
 **************************************************/
bool reduceOp(void) {
    expr_t *rhsExpr;
    expr_t *pe;
    ...
    case T_SIZEOF:
        ...
        if (lhsExpr->tType != S_TYPE && lhsExpr->tType != T_ID) {
            ...
            if (lhsExpr->attr.nodeType == EXPRNODE) {
                ...
                rhsExpr = convertToType(cloneExpr(lhsExpr->attr.pExpr), DT_UINT);
                op      = T_MUL;
            }
            freeExpr(lhsExpr);
            lhsExpr = pe;
...
/**************************************************
 * 24: 1340 PMO +++
 * resolveMember - Resolve struct/union member access
 **************************************************/
expr_t *resolveMember(register expr_t *st, expr_t *p2) {
    sym_t *foundMember;
    sym_t *structSym;
...
/**************************************************
 * 30: 1CCC PMO +++
 * convertToType - Convert expression to specified data type
 **************************************************/
expr_t *convertToType(expr_t *p1, uint8_t newDt) {
    attr_t st;
...
/**************************************************
 * 31: 1D02 PMO +++
 * promoteArg - Promote argument type for function calls
 **************************************************/
expr_t *promoteArg(register expr_t *st) {
...
/**************************************************
 * 33: 1DF0 PMO +++
 * funcToAddr - Convert function type to address
 **************************************************/
expr_t *funcToAddr(register expr_t *st) {
...
/**************************************************
 * 34: 1E37 PMO +++
 * arrayDecay - Handle array decay to pointer
 **************************************************/
expr_t *arrayDecay(register expr_t *st) {
...
/**************************************************
 * 36: 1EBD PMO +++
 * sizeofDeref - Create sizeof for dereferenced pointer
 **************************************************/
expr_t *sizeofDeref(register expr_t *st) {
...
/**************************************************
 * 37: 1EF1 PMO +++
 * isLvalue - Check if expression is an lvalue
 **************************************************/
bool isLvalue(register expr_t *st) {
...
/**************************************************
 * 38: 1F5D PMO +++
 * assignType - Assign expression to type with checks
 **************************************************/
expr_t *assignType(register expr_t *src, attr_t *dAttr, bool unscaled) {
    attr_t *sAttr;
...
/**************************************************
 * 55: 25F7 PMO +++
 * incrExpr - Increment constant expression by 1
 **************************************************/
