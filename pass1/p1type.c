...
/**************************************************
 * 134: 5C50 PMO +++
 * parseDeclStmt - Parse single declaration statement
 **************************************************/
void parseDeclStmt(void) {
    uint8_t scType;
    attr_t attr;
    ...
    for (;;) {
        ...
        if ((tok = yylex()) == T_EQ) {
            if (isFunc || scType == D_STACK || scType == T_EXTERN || scType == T_TYPEDEF)
...
/**************************************************
 * 135: 5DD1 PMO +++
 * parseTypeSpec - Parse type specifiers (storage class, base type)
 **************************************************/
            ...
            do {
                ...
                if ((tok = yylex()) == T_COLON) {
                    ...
                    if ((tok = yylex()) != T_ICONST)
                        ...
                        if (!pSym) {
                            defineArg(pSym = declareSym(newTmpSym(), D_MEMBER, &attr, st));
                            if (ppMembers) {
                                *ppMembers = pSym;
                                ppMembers  = &pSym->memberList;
                            }
...
/**************************************************
 * 137: 6360 PMO +++
 * parseEnumDef - Parse enum type definition
 **************************************************/
    ...
    if (tok == T_LBRACE) {
        ...
        for (;;) {
            if ((tok = yylex()) != T_ID) {
                ...
                if ((constSym = declareSym(yylval.ySym, DT_CONST, &enumAttr, st)))
                    constSym->memberId = enumIdx++;
                if ((tok = yylex()) == T_EQ) {
                    freeExpr(valExpr);
                    parseConstExpr(T_LBRACE);
                    if (!isIntType(&valExpr->attr) || valExpr->attr.dataType >= DT_LONG)
...
/**************************************************
 * 138: 6531 PMO +++
 * parseAutoInit - Parse auto variable initializer
 **************************************************/
void parseAutoInit(register sym_t *st) {
    expr_t *initExpr;
    uint8_t tok;
    ...
    if (st && !isLogicalType(&st->attr)) {
        ...
        if (!(hasBrace = (tok = yylex()) == T_LBRACE))
            ungetTok = tok;
        if ((initExpr = parseExpr(T_EROOT, parseExprMode(T_RBRACE), 0)) && st) {
...
sym_t *parseDeclr(uint8_t p1, register attr_t *p2, uint8_t p3, sym_t *p4) {
    ...
    if (curDecl->pSym2) {
        curDecl->pSym2 = declareSym(curDecl->pSym2, T_TYPEDEF, &curDecl->pSym2->attr, 0);
    ...
    if (curDecl->pSym1) { /* 6e0b */
        if (defSClass == D_STACK && p1 != D_MEMBER) {
            ...
            if (p1 == T_AUTO && attr.nodeType == FUNCNODE)
                p1 = T_EXTERN;
            if ((curDecl->pSym1 = declareSym(curDecl->pSym1, p1, &attr, p4)))
