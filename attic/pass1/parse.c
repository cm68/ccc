 ...
 * Phase 2 retrieves them in reverse order (LIFO) because nested
 * structures are pushed depth-first but emitted parent-first.
 */
static unsigned char countBuf[MAX_COUNTS];
static int countTop = 0;   /* stack pointer (grows up, LIFO pop) */
...
statement(struct stmt *parent)
{
    ...
    while (block) {
        ...
        if (phase == 1) {
            ...
            case END:
            case E_O_F:
                /* Push statement count for this block before exiting */
            ...
            case BEGIN:
                gettoken();
                pushScope(blockname());
                statement(0);  /* recurse - will push its own count */
                popScope();
                expect(END, ER_S_CC);
            ...
            case SWITCH:
                ...
                break;
            case CASE:
                /* Case labels are part of switch, counted there */
                gettoken();
                parseExpr(13, parent);
