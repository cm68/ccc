 ...
 * so the file builds with ccc -m cpm against an ordinary libc.  Define
 * WSLIB to get the original calls back.
 */
...
main (ac, av)
	int	ac;
	/*
	 * "char *av[]" is what this said, and what Whitesmiths compiled
	 * in 1982.  It was spelled "char **av" by hand for a while
	 * because cpp's K&R normalizer had no case for an array
	 * parameter and gave up on it, leaving ANSI-only c0 looking at
	 * raw K&R.  It decays properly now, so the original stands - and
	 * this file is where that is checked on something real.
	 */
	char	*av[];
	{
	...
	while (ac--)
		...
		if (cmpstr (av[0], "size"))
			...
			if (ac)
				...
				/*
				 * "switch (size)" over 128,256,512,1024,2048 is
				 * what this said.  ccc dispatches a switch on
				 * eight bits - see src/pass2/CONDITIONS.md -
				 * so those labels do not fit, and four of them
				 * mask to the same zero.
				 *
				 * Every legal size is 128 doubled, so switch on
				 * how many 128s and the labels become 1,2,4,8,
				 * 16, which do fit.  The remainder test is not
				 * decoration: without it 200/128 is 1 and a 200
				 * byte sector would format as 128.  Anything
				 * that is not an exact multiple yields 0, which
				 * no arm names, so it lands on the default
				 * exactly as an unrecognised size did before.
				 */
