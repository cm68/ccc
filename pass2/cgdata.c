...
member_t *typeLong; /* "l" - long		   */
int funcLocalSize[MAXFUN];                    /* local frame size per function */
...
int lvlidx; /* Level nested function   */

int emitOffset;           /* emitCodePat, emitNodeCode */
char lastResultReg;       /* calcUsedRegs, getResultReg */
int exprNestDepth;        /* emitCodePat, emitExprTree */
node_t *deferList[0x14];  /* deferPostInc, emitExpr */
void *nodeFreeList;       /* freeNode, allocNode, relNodeFrList */
bool treeChanged;         /* optimization loop flag */
uint8_t deferCnt;         /* deferPostInc, emitExpr */
char *warningMsg;         /* Pointer for printf      */
int availRegs;            /* Available register mask */
 ...
 * See cgen.h for DOPE_* mask definitions.
 *
 ...
 * for tracking register allocation and conflicts.
 *
 * Indices 0-10: Single register bitmasks (powers of 2)
 *   0=none, 1=a(0x01), 2=c(0x02), 3=b(0x04), 4=e(0x08),
 *   5=d(0x10), 6=l(0x20), 7=h(0x40), 8=ix(0x80),
 *   9=iy(0x100), 10=sp(0x200)
 *
 * Indices 11+: Register group combinations
 ...
 * Returns the component register index:
 *   n=0: low byte register, n=1: high byte register
 *
 * Examples (using regNames indices):
 *   bc(12): [24]=c(2), [25]=b(3)
 *   de(13): [26]=e(4), [27]=d(5)
 *   hl(14): [28]=l(6), [29]=h(7)
 *********************************************************/
 ...
 * (for class codes 24+). Zero-terminated within each group.
 * Indexed as &regClassRegs[(classCode - 24) * 6].
 *
 * Class 24: hl,de,bc      (word registers)
 * Class 25: de,bc         (word regs excl hl)
 * Class 26: a,e,d,c,b,l   (byte registers)
