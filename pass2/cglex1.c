...
/*
 * Code generation dispatch tables for operator-to-code-fragment mapping.
 *
 * opEmitOffsets: Index by operator type (0-71) to get offset into opEmitCodes
 * opEmitCodes:   Null-terminated strings of valid emit query codes per operator
 * opEmitValues:  Values returned for each position in opEmitCodes:
 *                - positive: starting index into codeFragLists, which contains
 *                  lists of codeFrag indices (negative value ends each list)
 *                - negative: negated index directly into codeFrag[] table
 *                - zero: no match / end of string
 *
 * lookupEmitCode(query, op) searches opEmitCodes[opEmitOffsets[op]...] for query char,
 * returns corresponding opEmitValues entry, or 0 if not found.
 *
 * Used by matchEmitPat to find code generation patterns for expression nodes.
 */

/* offsets into opEmitCodes for each operator type (0-71) */
static int16_t opEmitOffsets[] = {
                                0,     0,     4,     9,     0x0A,  0x0C,  0x0E,  0x1C,  0x2D,
...
/* valid emit query codes for each operator - null-terminated strings */
static uint8_t opEmitCodes[] = {
    "Qhi\0"                           /* [0 or 1] 000h */
    "PQhi\0"                          /* [2] 004h */
    "\0"                              /* [3] 009h */
    "H\0"                             /* [4] 00Ah */
...
/* code fragment indices corresponding to each position in opEmitCodes */
static int16_t opEmitValues[] = {
...
int lookupEmitCode(int queryCode, int opType) {
    ...
    for (idx = opEmitOffsets[opType]; opEmitCodes[idx] != 0 && opEmitCodes[idx] != queryCode; idx++)
        ;
    return opEmitValues[idx];
}
