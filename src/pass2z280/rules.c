...
#define F_CPWHLDE      "\311"	/*  cpw hl,de */
#define F_CPWHLBC      "\312"	/*  cpw hl,bc */
#define F_CPWR         "\313"	/*  cpw hl,$R */
#define F_XORH         "\314"	/*  xor h */
#define F_XORD         "\315"	/*  xor d */
#define F_XORB         "\316"	/*  xor b */
#define F_LDWHLI       "\317"	/*  ld hl,(hl) */
#define F_LDWHLDE      "\320"	/*  ld (hl),de */
...
#define T_IXP_LD	"\tld l,($i+0)\n\tld h,($i+1)\n"
#define T_IXP_ST	"\tld ($i+0),l\n\tld ($i+1),h\n"
...
/*
 * The same signed fold via CPW.  cpw sets C to the unsigned borrow
 * (HL < src) and leaves HL and DE untouched, so the operands' sign
 * bits stay readable.  A signed comparison is that borrow xored with
 * the two sign bits:  a < b  =  C ^ sa ^ sb.  sbc a,a turns C into A
 * (all bits), the two xors fold in the sign bits, and or a reads the
 * answer off bit 7.  Six bytes against T_SXORV's ten, and no result
 * has to be materialised - which is what CPW takes away.
 */
#define T_SF_DE	F_CPWHLDE F_SBCAA F_XORH F_XORD
#define T_SF_BC	F_CPWHLBC F_SBCAA F_XORH F_XORB
/*
 * The signed fold against a constant whose sign is known at compile
 * time.  Positive: sb is 0, so neither the sign bit of the constant
 * nor a register to hold it is needed - cpw hl,n sets C straight off
 * and the fold drops its xor.  The swap forms keep the ex de,hl and
 * drop xor h, which reads the (zero) sign of the constant.
 */
#define T_SF_POS	F_CPWR F_SBCAA F_XORH
#define T_SF_POS_BC	T_BC_HL F_CPWR F_SBCAA F_XORH
#define T_SF_POS_SWAP	F_LDDER F_EXDEHL F_CPWHLDE F_SBCAA F_XORD
#define T_SF_PSWAP_BC	T_BC_HL F_LDDER F_EXDEHL F_CPWHLDE F_SBCAA F_XORD
/*
...
#define T_ST_IHL	F_LDWHLDE F_EXDEHL
...
static char RT162[] = "\tld $i,$R\n";
static char RT170[] = "\tor e\n";
static char RT174[] = "\tpush $i\n" F_POPHL;
static char RT178[] = "\tld hl,$R\n\tcpw hl,$i\n";
static char RT179[] = "\tpush $i\n" F_POPHL F_CPWR;
static char RT183[] = "\tpush $i\n" F_POPHL F_LDDER F_ORA F_SBCHLDE;
static char RT184[] = "\tpush $i\n" F_POPHL F_CPWHLBC;
static char RT185[] = "\tpush $i\n" F_POPHL F_CPWHLBC;
static char RT185S[] = "\tpush $i\n" F_POPHL F_ORA F_SBCHLBC;
static char RT186[] = "\tpush $i\n" F_POPHL F_CPWHLDE;
static char RT186S[] = "\tpush $i\n" F_POPHL F_ORA F_SBCHLDE;
static char RT187[] = "\tpush $i\n" F_POPHL T_HL_TEST;
static char RT193[] = "\tcpw hl,$i\n";
static char RT193S[] = "\tpush $i\n\tpop de\n" F_ORA F_SBCHLDE;
...
static char RT285[] = F_LDDER F_EXDEHL T_SF_DE;
static char RT287[] = F_CPWR;
static char RT287S[] = F_LDDER F_ORA F_SBCHLDE;
static char RT288[] = F_CPWR F_JRNZ3 "\tscf\n";
static char RT290[] = F_LDDER T_SF_DE;
static char RT300[] = F_LDHLL "\tdec (hl)\n";
static char RT302[] = F_LDHLL "\tinc (hl)\n";
static char RT31[] = "\tadd a,e\n";
static char RT312[] = F_LDHLL F_CPWHLDE;
static char RT313[] = F_LDHLL F_CALLLADEC;
static char RT315[] = F_LDHLL F_CALLLAINC;
static char RT317[] = F_LDHLL T_SF_BC;
static char RT318[] = F_LDHLL T_SF_DE;
...
static char RT359[] = F_CPWHLBC;
static char RT359S[] = F_ORA F_SBCHLBC;
static char RT360[] = F_CPWHLDE;
static char RT360S[] = F_ORA F_SBCHLDE;
...
static char RT396[] = T_BC_HL "\tcpw hl,$i\n";
static char RT396S[] = T_BC_HL "\tpush $i\n\tpop de\n" F_ORA F_SBCHLDE;
static char RT41[] = "\tcp b\n";
static char RT411[] = T_BC_HL F_CPWR;
static char RT411S[] = T_BC_HL F_LDDER F_ORA F_SBCHLDE;
static char RT412[] = T_BC_HL F_CPWR F_JRNZ3 "\tscf\n";
static char RT413[] = T_BC_HL F_LDDER T_SF_DE;
static char RT415[] = T_BC_HL F_CPWHLDE;
static char RT415S[] = T_BC_HL F_ORA F_SBCHLDE;
static char RT416[] = T_BC_HL F_CALLLADEC;
static char RT418[] = T_BC_HL F_CALLLAINC;
static char RT42[] = "\tcp c\n";
static char RT425[] = T_BC_HL T_SF_DE;
...
struct rule rules[] = {
	...
	R(BANG,REGVAR,0,0,0,0, BANG, P_L, P_NONE, P_L, RF_IXIY, RT443, F_Z),
	...
	R(PLUS,REGVAR,P_NUM,0,0,0, INDEX, P_NONE, P_NONE, P_L, RF_IXIY, 0, 0),
	 ...
	 */
	R(PLUS,REGVAR,P_NUM,0,0,0, CODE, P_NONE, P_NONE, P_L, RF_IXIY, 0, 0),
	...
	R(PLUS,REGVAR,SYMREF,0,0,0, PLUS, P_L, P_R, P_L, RF_IXIY,
	 ...
	 */
	R(PLUS,REGVAR,INDE,0,0,0, PLUS, P_L, P_R, P_L, RF_IXIY,
		"\tpush $i\n" F_POPHL F_ADDHLDE, R_HL),
	R(PLUS,REGVAR,INBC,0,0,0, PLUS, P_L, P_R, P_L, RF_IXIY,
		"\tpush $i\n" F_POPHL "\tadd hl,bc\n", R_HL),
	R(PLUS,INBC,REGVAR,0,0,0, PLUS, P_L, P_R, P_R, RF_IXIY,
		T_BC_HL "\tpush $i\n\tpop de\n" F_ADDHLDE, R_HL),
	R(PLUS,INHL,REGVAR,0,0,0, PLUS, P_L, P_R, P_R, RF_IXIY,
		"\tpush $i\n\tpop de\n" F_ADDHLDE, R_HL),
	 ...
	 */
	R(MINUS,REGVAR,P_NUM,0,0,0, MINUS, P_L, P_R, P_L, RF_IXIY, RT183, R_HL),
	R(MINUS,REGVAR,SYMREF,0,0,0, MINUS, P_L, P_R, P_L, RF_IXIY, RT183, R_HL),
	/*
	 ...
	 * source wrote it.
	 */
	R(MINUS,SYMREF,REGVAR,0,0,0, MINUS, P_L, P_R, P_R, RF_IXIY,
	...
	R(MINUS,REGVAR,INDE,0,0,0, MINUS, P_L, P_R, P_L, RF_IXIY, RT186S, R_HL),
	/*
	 * Two pointers both in index registers: "p - q" with p in IX and
	 * q in IY - possible now that IY is allocatable.  Move each into
	 * its subtract register and sbc.
	 */
	R(MINUS,REGVAR,REGVAR,0,0,0, MINUS, P_L, P_R, P_L, RF_IXIY,
	 ...
	 */
	R(MINUS,REGVAR,INBC,0,0,0, MINUS, P_L, P_R, P_L, RF_IXIY, RT185S, R_HL),
	R(MINUS,INBC,REGVAR,0,0,0, MINUS, P_L, P_R, P_R, RF_IXIY, RT396S, R_HL),
	/*
	 ...
	 */
	R(MINUS,INHL,REGVAR,0,0,0, MINUS, P_L, P_R, P_R, RF_IXIY, RT193S, R_HL),
	R(MINUS,INHL,INBC,0,0,0, MINUS, P_L, P_R, P_NONE, 0, RT359S, R_HL),
	...
	R(MINUS,P_NUM,INA,0,0,1, MINUS, P_L, P_R, P_NONE, 0,
		"\tld e,a\n\tld a,$L\n" F_SUBE, R_A),
	R(MINUS,INBC,INDE,0,0,0, MINUS, P_L, P_R, P_NONE, 0, RT415S, R_HL),
	/*
	 ...
	 */
	R(MINUS,INBC,SYMREF,0,0,0, MINUS, P_L, P_R, P_NONE, 0, RT411S, R_HL),
	R(MINUS,INBC,P_NUM,0,0,0, MINUS, P_L, P_R, P_NONE, 0, RT411S, R_HL),
	...
	R(MINUS,INHL,INDE,0,0,0, MINUS, P_L, P_R, P_NONE, 0, RT360S, R_HL),
	/*
	...
	R(MINUS,INHL,SYMREF,0,0,0, MINUS, P_L, P_R, P_NONE, 0, RT287S, R_HL),
	R(MINUS,INHL,P_NUM,0,0,0, MINUS, P_L, P_R, P_NONE, 0, RT287S, R_HL),
	...
	R(AND,DEREF,P_POW2,REGVAR,0,9, AND, P_L, P_R, P_LL, RF_POW2 | RF_IXIY,
		"\tbit $R,($i+0)\n", F_NZ),
	 ...
	 */
	R(EQ,REGVAR,P_ZERO,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT187, F_Z),
	/*
	 ...
	 */
	R(EQ,REGVAR,P_NUM,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT179, F_CC),
	R(EQ,REGVAR,SYMREF,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT179, F_CC),
	R(EQ,REGVAR,INBC,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT184, F_Z),
	R(EQ,REGVAR,INHL,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT193, F_Z),
	/*
	 ...
	 */
	R(EQ,REGVAR,INDE,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT186, F_CC),
	...
	R(EQ,INHL,REGVAR,0,0,0, EQ, P_L, P_R, P_R, RF_IXIY, RT193, F_CC),
	 ...
	 */
	R(EQ,INBC,REGVAR,0,0,0, EQ, P_L, P_R, P_R, RF_IXIY, RT396, F_CC),
	/* and mirrored, the IX pointer on the left */
	R(EQ,REGVAR,INBC,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT185, F_CC),
	/* index register against index register: two pointer register
	 * variables compared, possible only now that IY is one */
	R(EQ,REGVAR,REGVAR,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY,
		"\tpush $i\n\tpop hl\n\tpush $Rr\n\tpop de\n" F_CPWHLDE, F_CC),
	...
	R(NEQ,REGVAR,P_ZERO,0,0,0, NEQ, P_L, P_R, P_L, RF_IXIY, RT187, F_NZ),
	R(NEQ,REGVAR,P_NUM,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT179, F_CC),
	R(NEQ,REGVAR,SYMREF,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT179, F_CC),
	R(NEQ,REGVAR,INBC,0,0,0, NEQ, P_L, P_R, P_L, RF_IXIY, RT184, F_NZ),
	R(NEQ,REGVAR,INHL,0,0,0, NEQ, P_L, P_R, P_L, RF_IXIY, RT193, F_NZ),
	R(NEQ,REGVAR,INDE,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT186, F_CC),
	R(NEQ,INHL,REGVAR,0,0,0, EQ, P_L, P_R, P_R, RF_IXIY, RT193, F_CC),
	R(NEQ,INBC,REGVAR,0,0,0, EQ, P_L, P_R, P_R, RF_IXIY, RT396, F_CC),
	R(NEQ,REGVAR,INBC,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT185, F_CC),
	...
	R(LE,REGVAR,P_NUM,0,0,0, LE, P_L, P_R, P_L, RF_IXIY, RT178,
		F_CC),
	R(LE,REGVAR,SYMREF,0,0,0, LE, P_L, P_R, P_L, RF_IXIY, RT178,
		F_CC),
	R(LE,REGVAR,INDE,0,0,0, LE, P_L, P_R, P_L, RF_IXIY, "\tpush $i\n" F_POPHL F_EXDEHL F_CPWHLDE, F_CC),
	R(LE,INHL,REGVAR,0,0,0, LE, P_L, P_R, P_R, RF_IXIY, "\tpush $i\n\tpop de\n" F_EXDEHL F_CPWHLDE, F_CC),
	R(LE,INBC,REGVAR,0,0,0, LE, P_L, P_R, P_R, RF_IXIY, T_BC_HL "\tpush $i\n\tpop de\n" F_EXDEHL F_CPWHLDE, F_CC),
	R(LE,REGVAR,INBC,0,0,0, LE, P_L, P_R, P_L, RF_IXIY, RT396, F_CC),
	R(LE,REGVAR,REGVAR,0,0,0, LE, P_L, P_R, P_L, RF_IXIY,
		"\tpush $Rr\n\tpop hl\n\tpush $i\n\tpop de\n" F_CPWHLDE, F_CC),
	R(LE,INHL,P_ZERO,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, RT229, F_Z),
	R(LE,INBC,P_ZERO,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, RT220, F_Z),
	R(LE,INHL,INDE,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, F_EXDEHL T_SF_DE, F_CC),
	R(LE,SYMREF,INDE,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, F_LDHLL F_EXDEHL T_SF_DE, F_CC),
	R(LE,SYMREF,INBC,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, F_LDHLL F_LDEC F_LDDB F_EXDEHL T_SF_DE, F_CC),
	R(LE,INHL,SYMREF,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, RT285, F_CC),
	R(LE,INHL,INBC,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, F_LDEC F_LDDB F_EXDEHL T_SF_DE, F_CC),
	R(LE,INHL,P_POS,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, T_SF_POS_SWAP, F_CC),
	...
	R(LE,INHL,INDE,0,0,0, LE, P_L, P_R, P_NONE, 0, F_EXDEHL F_CPWHLDE, F_CC),
	/* the same four with a symbol on the left - see the signed set */
	R(LE,SYMREF,INDE,0,0,0, LE, P_L, P_R, P_NONE, 0, F_LDHLL F_EXDEHL F_CPWHLDE, F_CC),
	R(LE,INHL,SYMREF,0,0,0, LE, P_L, P_R, P_NONE, 0, F_LDDER F_EXDEHL F_CPWHLDE, F_CC),
	/*
	 * LE and GT answer the reversed question, and there is no
	 * ex bc,hl - so copy BC into DE and swap that instead.
	 */
	R(LE,INHL,INBC,0,0,0, LE, P_L, P_R, P_NONE, 0, F_LDEC F_LDDB F_EXDEHL F_CPWHLDE, F_CC),
	R(LE,INBC,INDE,0,0,0, GT, P_L, P_R, P_NONE, RF_SIGNL, T_BC_HL F_EXDEHL T_SF_DE, F_CC),
	R(LE,INBC,P_POS,0,0,0, GT, P_L, P_R, P_NONE, RF_SIGNL, T_SF_PSWAP_BC, F_CC),
	R(LE,INBC,P_NUM,0,0,0, GT, P_L, P_R, P_NONE, RF_SIGNL, T_BC_HL F_LDDER F_EXDEHL T_SF_DE, F_CC),
	...
	R(LE,INBC,INDE,0,0,0, GT, P_L, P_R, P_NONE, 0, T_BC_HL F_EXDEHL F_CPWHLDE, F_CC),
	R(LT,REGVAR,P_NUM,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT179, F_CC),
	R(LT,REGVAR,SYMREF,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT179, F_CC),
	R(LT,REGVAR,INDE,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT186, F_CC),
	R(LT,INHL,REGVAR,0,0,0, EQ, P_L, P_R, P_R, RF_IXIY, RT193, F_CC),
	R(LT,INBC,REGVAR,0,0,0, EQ, P_L, P_R, P_R, RF_IXIY, RT396, F_CC),
	R(LT,REGVAR,INBC,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT185, F_CC),
	R(LT,REGVAR,REGVAR,0,0,0, LT, P_L, P_R, P_L, RF_IXIY,
		"\tpush $i\n\tpop hl\n\tpush $Rr\n\tpop de\n" F_CPWHLDE, F_CC),
	...
	R(LT,INHL,P_POS,0,0,0, LT, P_L, P_R, P_NONE, RF_SIGNL, T_SF_POS, F_M),
	...
	R(LT,INBC,P_POS,0,0,0, LT, P_L, P_R, P_NONE, RF_SIGNL, T_SF_POS_BC, F_M),
	...
	R(GE,REGVAR,P_NUM,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT179, F_CC),
	R(GE,REGVAR,SYMREF,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT179, F_CC),
	R(GE,REGVAR,INDE,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT186, F_CC),
	R(GE,INHL,REGVAR,0,0,0, EQ, P_L, P_R, P_R, RF_IXIY, RT193, F_CC),
	R(GE,INBC,REGVAR,0,0,0, EQ, P_L, P_R, P_R, RF_IXIY, RT396, F_CC),
	R(GE,REGVAR,INBC,0,0,0, EQ, P_L, P_R, P_L, RF_IXIY, RT185, F_CC),
	...
	R(GE,INHL,P_POS,0,0,0, GE, P_L, P_R, P_NONE, RF_SIGNL, T_SF_POS, F_P),
	...
	R(GE,INBC,P_POS,0,0,0, GE, P_L, P_R, P_NONE, RF_SIGNL, T_SF_POS_BC, F_P),
	...
	R(GT,REGVAR,P_NUM,0,0,0, LE, P_L, P_R, P_L, RF_IXIY, RT178,
		F_CC),
	R(GT,REGVAR,SYMREF,0,0,0, LE, P_L, P_R, P_L, RF_IXIY, RT178,
		F_CC),
	R(GT,REGVAR,INDE,0,0,0, LE, P_L, P_R, P_L, RF_IXIY, "\tpush $i\n" F_POPHL F_EXDEHL F_CPWHLDE, F_CC),
	R(GT,INHL,REGVAR,0,0,0, LE, P_L, P_R, P_R, RF_IXIY, "\tpush $i\n\tpop de\n" F_EXDEHL F_CPWHLDE, F_CC),
	R(GT,INBC,REGVAR,0,0,0, LE, P_L, P_R, P_R, RF_IXIY, T_BC_HL "\tpush $i\n\tpop de\n" F_EXDEHL F_CPWHLDE, F_CC),
	R(GT,REGVAR,INBC,0,0,0, LE, P_L, P_R, P_L, RF_IXIY, RT396, F_CC),
	/*
	...
	R(GT,INHL,INDE,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, F_EXDEHL T_SF_DE, F_CC),
	R(GT,SYMREF,INDE,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, F_LDHLL F_EXDEHL T_SF_DE, F_CC),
	R(GT,SYMREF,INBC,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, F_LDHLL F_LDEC F_LDDB F_EXDEHL T_SF_DE, F_CC),
	R(GT,INHL,SYMREF,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, RT285, F_CC),
	R(GT,INHL,INBC,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, F_LDEC F_LDDB F_EXDEHL T_SF_DE, F_CC),
	R(GT,INHL,P_POS,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, T_SF_POS_SWAP, F_CC),
	R(GT,INHL,P_NUM,0,0,0, LE, P_L, P_R, P_NONE, RF_SIGNL, RT285, F_CC),
	R(GT,INHL,INDE,0,0,0, LE, P_L, P_R, P_NONE, 0, F_EXDEHL F_CPWHLDE, F_CC),
	R(GT,SYMREF,INDE,0,0,0, LE, P_L, P_R, P_NONE, 0, F_LDHLL F_EXDEHL F_CPWHLDE, F_CC),
	R(GT,INHL,SYMREF,0,0,0, LE, P_L, P_R, P_NONE, 0, F_LDDER F_EXDEHL F_CPWHLDE, F_CC),
	R(GT,INHL,INBC,0,0,0, LE, P_L, P_R, P_NONE, 0, F_LDEC F_LDDB F_EXDEHL F_CPWHLDE, F_CC),
	R(GT,INBC,INDE,0,0,0, GT, P_L, P_R, P_NONE, RF_SIGNL, T_BC_HL F_EXDEHL T_SF_DE, F_CC),
	R(GT,INBC,P_POS,0,0,0, GT, P_L, P_R, P_NONE, RF_SIGNL, T_SF_PSWAP_BC, F_CC),
	R(GT,INBC,P_NUM,0,0,0, GT, P_L, P_R, P_NONE, RF_SIGNL, T_BC_HL F_LDDER F_EXDEHL T_SF_DE, F_CC),
	...
	R(GT,INBC,INDE,0,0,0, GT, P_L, P_R, P_NONE, 0, T_BC_HL F_EXDEHL F_CPWHLDE, F_CC),
	...
	/* copy IX to HL/BC/DE */
	R(ASSIGN,INHL,REGVAR,0,0,0, ASSIGN, P_L, P_R, P_R, RF_IXIY, RT174, R_HL),
	R(ASSIGN,INBC,REGVAR,0,0,0, ASSIGN, P_L, P_R, P_R, RF_IXIY, "\tld c,$I\n\tld b,$J\n", R_BC),
	R(ASSIGN,INDE,REGVAR,0,0,0, ASSIGN, P_L, P_R, P_R, RF_IXIY, "\tld e,$I\n\tld d,$J\n", R_DE),
	...
	R(ASSIGN,REGVAR,P_NUM,0,0,0, ASSIGN, P_L, P_R, P_L, RF_IXIY, RT162, R_IX),
	 ...
	 */
	R(ASSIGN,REGVAR,SYMREF,0,0,0, ASSIGN, P_L, P_R, P_L, RF_IXIY, RT162, R_IX),
	R(ASSIGN,SYMREF,REGVAR,0,0,0, ASSIGN, P_L, P_R, P_R, RF_IXIY, "\tld ($L),$i\n", R_IX),
	/*
	 ...
	 */
	R(ASSIGN,INDEX,REGVAR,0,0,2, ASSIGN, P_L, P_R, P_R, RF_IXIY,
	 ...
	 */
	R(ASSIGN,REGVAR,INDEX,0,0,0, ASSIGN, P_L, P_R, P_L, RF_IXIY,
		"\tlda hl,($R)\n" F_PUSHHL "\tpop $i\n", R_IX),

	R(ASSIGN,REGVAR,INHL,0,0,0, ASSIGN, P_L, P_R, P_L, RF_IXIY, F_PUSHHL "\tpop $i\n", R_IX),
	R(ASSIGN,REGVAR,INDE,0,0,0, ASSIGN, P_L, P_R, P_L, RF_IXIY, "\tpush de\n\tpop $i\n", R_IX),
	R(ASSIGN,REGVAR,INBC,0,0,0, ASSIGN, P_L, P_R, P_L, RF_IXIY, F_PUSHBC "\tpop $i\n", R_IX),
	/* index register to index register: only two index-register
	 * variables can meet this way, and only now that IY is one */
	R(ASSIGN,REGVAR,REGVAR,0,0,0, ASSIGN, P_L, P_R, P_L, RF_IXIY,
		"\tpush $Rr\n\tpop $Lr\n", R_IX),
	...
	R(ASSIGN,DEREF,SYMREF,INBC,0,2, ASSIGN, P_L, P_R, P_NONE, 0,
		F_LDDER F_LDLC F_LDHB F_LDWHLDE, R_DE),
	R(ASSIGN,DEREF,SYMREF,INDEX,0,2, ASSIGN, P_L, P_R, P_LL, RF_IX,
		F_LDDER "\tld l,($LL)\n\tld h,($LL+)\n"
		F_LDWHLDE, R_DE),
	R(ASSIGN,DEREF,SYMREF,INDEX,0,2, ASSIGN, P_L, P_R, P_NONE, 0,
		F_LDDER F_LDHLL4 F_LDWHLDE, R_DE),
	 ...
	 * is back in HL.
	 */
	R(ASSIGN,DEREF,INHL,REGVAR,0,3, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
		F_PUSHHL "\tpush $i\n" F_POPHL "\tex (sp),hl\n"
		"\tcall qst\n", R_HL),
	R(ASSIGN,DEREF,P_NUM,REGVAR,0,3, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
	...
	R(ASSIGN,DEREF,INHL,INBC,0,2, ASSIGN, P_L, P_R, P_NONE, 0, F_PUSHHL F_LDLC F_LDHB F_POPDE F_LDWHLDE, 0),
	 ...
	 */
	R(ASSIGN,DEREF,P_NUM,REGVAR,0,2, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
		"\tld ($i+0),$Rl\n\tld ($i+1),$Rh\n", 0),
	/* destval says so because it is true: the store reads HL and
	 * leaves it - which is what lets "head = p->next = q" chain */
	R(ASSIGN,DEREF,INHL,REGVAR,0,2, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
		"\tld ($i+0),l\n\tld ($i+1),h\n", R_HL),
	/*
	 ...
	 */
	R(ASSIGN,DEREF,SYMREF,REGVAR,0,2, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
		F_LDDER "\tld ($i+0),e\n\tld ($i+1),d\n", R_DE),
	/*
	 ...
	 */
	R(ASSIGN,DEREF,INBC,REGVAR,0,2, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
		"\tld ($i+0),c\n\tld ($i+1),b\n", R_BC),
	/* one index-register pointer written through another: "*p = q"
	 * with p in IX and q in IY - two register pointers meeting, only
	 * possible now that IY is allocatable.  Bring the value into HL
	 * and write it out, as the INDEX store above does. */
	R(ASSIGN,DEREF,REGVAR,REGVAR,0,2, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
		"\tpush $Rr\n" F_POPHL F_LDAL "\tld ($i+0),a\n" F_LDAH
		"\tld ($i+1),a\n", R_HL),
	/*
	 ...
	 */
	R(ASSIGN,DEREF,INDE,REGVAR,0,2, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
		"\tld ($i+0),e\n\tld ($i+1),d\n", R_DE),
	R(ASSIGN,DEREF,P_NUM,REGVAR,0,1, ASSIGN, P_L, P_R, P_LL, RF_IXIY, "\tld ($i+0),$R\n", 0),
	R(ASSIGN,DEREF,INA,REGVAR,0,1, ASSIGN, P_L, P_R, P_LL, RF_IXIY, "\tld ($i+0),a\n", R_A),
	/* a word in HL narrowed on its way through the index register */
	R(ASSIGN,DEREF,INHL,REGVAR,0,1, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
		F_LDAL "\tld ($i+0),a\n", R_A),
	/*
	 ...
	 */
	R(ASSIGN,DEREF,INBC,REGVAR,0,1, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
		F_LDAC "\tld ($i+0),a\n", R_A),
	R(ASSIGN,DEREF,INDE,REGVAR,0,1, ASSIGN, P_L, P_R, P_LL, RF_IXIY,
		"\tld a,e\n\tld ($i+0),a\n", R_A),
	 ...
	 * H and L to IXH and IXL for the whole instruction, so "ld l,$I"
	 ...
	 */
	R(ASSIGN,DEREF,REGVAR,INHL,0,2, ASSIGN, P_L, P_R, P_R, RF_IXIY,
		"\tpush $i\n\tpop de\n" F_LDWHLDE, R_DE),
	R(ASSIGN,DEREF,REGVAR,INHL,0,1, ASSIGN, P_L, P_R, P_R, RF_IXIY,
		"\tld a,$I\n" F_LDHLA, R_A),
	R(ASSIGN,DEREF,REGVAR,INBC,0,2, ASSIGN, P_L, P_R, P_R, RF_IXIY,
		F_PUSHHL T_BC_HL "\tpush $i\n\tpop de\n" F_LDHLE F_INCHL
		F_LDHLD F_POPHL, R_DE),
	R(ASSIGN,DEREF,REGVAR,INDEX,0,2, ASSIGN, P_L, P_R, P_LL, RF_IX,
		F_LDLLL F_LDHLL1 "\tpush $Rr\n\tpop de\n"
		F_LDWHLDE, R_DE),
	R(ASSIGN,DEREF,REGVAR,INDEX,0,2, ASSIGN, P_L, P_R, P_NONE, 0,
		F_LDHLL4 "\tpush $Rr\n\tpop de\n"
		F_LDWHLDE, R_DE),
	R(ASSIGN,DEREF,REGVAR,SYMREF,0,2, ASSIGN, P_L, P_R, P_R, RF_IXIY,
		F_PUSHHL "\tld hl,($LL)\n\tpush $i\n" F_POPDE
		F_LDWHLDE F_POPHL, R_DE),
	...
	R(ASSIGN,DEREF,INDE,INDEX,0,2, ASSIGN, P_L, P_R, P_LL, RF_IX,
		F_LDLLL F_LDHLL1 F_LDWHLDE, R_DE),
	R(ASSIGN,DEREF,INDE,INDEX,0,2, ASSIGN, P_L, P_R, P_NONE, 0,
		F_LDHLL4 F_LDWHLDE, R_DE),
	...
	R(ASSIGN,DEREF,INDE,INHL,0,2, ASSIGN, P_L, P_R, P_NONE, 0, F_LDWHLDE, 0),
	...
	R(DEREF,INHL,0,0,0,2, DEREF, P_L, P_NONE, P_NONE, 0, F_LDWHLI, R_HL),
	...
	R(DEREF,REGVAR,0,0,0,1, DEREF, P_L, P_NONE, P_L, RF_IXIY, "\tld a,($i+0)\n", R_A),
	R(DEREF,REGVAR,0,0,0,2, DEREF, P_L, P_NONE, P_L, RF_IXIY,
		"\tld $t,($i+0)\n\tld $u,($i+1)\n", 0),
	/*
	 ...
	 */
	R(DEREF,REGVAR,0,0,0,3, DEREF, P_L, P_NONE, P_L, RF_IXIY,
		F_EXX "\tld l,($i+0)\n\tld h,($i+1)\n" F_EXX
		"\tld l,($i+2)\n\tld h,($i+3)\n", R_HL),
	...
	R(WIDEN,REGVAR,0,0,0,2, WIDEN, P_L, P_NONE, P_L, RF_IXIY, RT174, R_HL),
	...
	R(SEXT,REGVAR,0,0,0,2, SEXT, P_L, P_NONE, P_L, RF_IXIY, RT174, R_HL),
	 ...
	 */
	R(PREINC,REGVAR,0,0,0,24, PREINC, P_L, P_NONE, P_L, RF_IXIY, RT67, 0),
	R(PREINC,REGVAR,0,0,0,0, PREINC, P_L, P_NONE, P_L, RF_IXIY,
		"\tinc $i\n\tpush $i\n" F_POPHL, R_HL),
	/*
	 ...
	 */
	R(PREINC,DEREF,0,REGVAR,0,2, PREINC, P_L, P_NONE, P_LL, RF_IXIY,
	...
	R(PREINC,DEREF,0,REGVAR,0,1, PREINC, P_L, P_NONE, P_LL, RF_IXIY,
		"\tinc ($i+0)\n\tld a,($i+0)\n", R_A),
	...
	R(POSTINC,REGVAR,0,0,0,24, POSTINC, P_L, P_NONE, P_L, RF_IXIY, RT67, 0),
	R(POSTINC,REGVAR,0,0,0,0, POSTINC, P_L, P_NONE, P_L, RF_IXIY,
		"\tpush $i\n" F_POPHL "\tinc $i\n", R_HL),
	R(POSTINC,DEREF,0,REGVAR,0,2, POSTINC, P_L, P_NONE, P_LL, RF_IXIY,
	 ...
	 */
	R(POSTINC,DEREF,0,REGVAR,0,1, POSTINC, P_L, P_NONE, P_LL, RF_IXIY,
		"\tld a,($i+0)\n\tinc ($i+0)\n", R_A),
	...
	R(PREDEC,REGVAR,0,0,0,24, PREDEC, P_L, P_NONE, P_L, RF_IXIY, RT54, 0),
	R(PREDEC,REGVAR,0,0,0,0, PREDEC, P_L, P_NONE, P_L, RF_IXIY,
		"\tdec $i\n\tpush $i\n" F_POPHL, R_HL),
	R(PREDEC,DEREF,0,REGVAR,0,2, PREDEC, P_L, P_NONE, P_LL, RF_IXIY,
	...
	R(PREDEC,DEREF,0,REGVAR,0,1, PREDEC, P_L, P_NONE, P_LL, RF_IXIY,
		"\tdec ($i+0)\n\tld a,($i+0)\n", R_A),
	...
	R(POSTDEC,REGVAR,0,0,0,24, POSTDEC, P_L, P_NONE, P_L, RF_IXIY, RT54, 0),
	R(POSTDEC,REGVAR,0,0,0,0, POSTDEC, P_L, P_NONE, P_L, RF_IXIY,
		"\tpush $i\n" F_POPHL "\tdec $i\n", R_HL),
	R(POSTDEC,DEREF,0,REGVAR,0,2, POSTDEC, P_L, P_NONE, P_LL, RF_IXIY,
	...
	R(POSTDEC,DEREF,0,REGVAR,0,1, POSTDEC, P_L, P_NONE, P_LL, RF_IXIY,
		"\tld a,($i+0)\n\tdec ($i+0)\n", R_A),
	...
	R(REGVAR,0,0,0,0,8, REGVAR, P_NONE, P_NONE, P_NONE, RF_IXIY, RT443, F_NZ),
