...
islocdesc(Expr *e)
{
	if (!e)
		return 0;
	switch (e->op) {
	/* A frame slot always names a place: SR mode reaches any offset
...
rewrite1(Expr *e)
{
	...
	/*
	 * DEREF(PLUS(ptr, NUMBER)): a word loaded through a pointer plus a
	 * constant offset.  The Z280's X mode takes HL as the base register,
	 * so once the pointer is in HL the whole walk is one ld hl,(hl+off).
	 * Only when the pointer really lands in HL; a register pointer keeps
	 * the byte pair, and the DE-target form still wants DE directly.
	 */
	if (e->op == DEREF && e->left && e->left->op == PLUS &&
	    e->left->right && e->left->right->op == NUMBER &&
	    (e->width | 0x20) == 's' && e->tgt != R_DE &&
	    !(e->left->nored & NR_NORED)) {
		Expr *ptr = e->left->left;
		Expr *p;
		long off = e->left->right->u.val;

		e->left->left = 0;	/* rewrite1 frees ptr */
		p = ptr ? rewrite1(ptr) : (Expr *)0;
		if (p && (p->op == INHL ||
		    (p->op == CODE && p->u.var.reg == R_HL))) {
			freeexpr(p);
			out("\tld hl,(hl+");
			outd(off);
			out(")\n");
