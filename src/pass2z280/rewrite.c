...
label(Expr *e)
{
	...
	/* A frame slot is a leaf whatever its offset: SR mode takes a
	 * 16-bit displacement, so no address arithmetic is ever needed */
	case LOCALVAR:
		e->regs = 1;
		return;
	...
	/*
	 * A pointer register plus a constant is a leaf: it becomes the
	 * (ix+d) or (iy+d) operand of whatever dereferences it, and X
	 * mode takes a 16-bit displacement, so no offset is out of
	 * reach.  Costs what a leaf costs.  (A word variable in BC is
	 * not a pointer register and stays on the Sethi-Ullman rule.)
	 */
	case PLUS:
		if (e->left && e->left->op == REGVAR &&
		    (e->left->u.var.reg == R_IX || e->left->u.var.reg == R_IY) &&
...
opmatch(unsigned char pat, Expr *e)
{
	...
	if (pat == P_POS) return e->op == NUMBER && (e->u.val & 0x8000) == 0;
