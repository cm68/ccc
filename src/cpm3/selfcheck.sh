...
for src in $srcs; do
	...
	( cd "$work" && "$top"/desthost/libexec/pass0 -DCCC -o h t.c ) >/dev/null 2>&1
	( cd "$work" && "$top"/desthost/libexec/c0 h.x h.ast h.dat ) >/dev/null 2>&1
	( cd "$work" && "$top"/desthost/libexec/c1 h.ast h.dat h.s ) >/dev/null 2>&1
	...
	if ! "$top"/desthost/libexec/asz "$work/t.s" -o "$work/t.o" >/dev/null 2>&1; then
		bad="$bad asm"
	fi
