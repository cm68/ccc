...
allocRegs(struct local *locals)
{
	...
	/*
	 * IX and then IY to the pointers with the highest ref_count.  The
	 * Z280's X mode makes *p, p->f and p + n all one (ix+d) operand, so
	 * any pointer site wants an index register - not just the field
	 * accesses agg_refs used to single out.  IY was the frame pointer;
	 * now that it is free it is the second index register.  An array
	 * carries TF_POINTER too - it decays - so the pointer bit alone
	 * does not mean there is a pointer here to put in a register: an
	 * array IS its storage, and giving it IX made "stack[0].l = 11"
	 * assign to the register while every field reference addressed the
	 * frame.
	 */
	{
		int i;
	...
	if (!(regs & 14)) {
		...
		for (n = locals; n; n = n->next) {
			...
			if (n->type->flags & (TF_ARRAY | TF_AGGREGATE))
				continue;
			/* a pointer prefers IX/IY above; BC is for integers */
			if (n->type->flags & TF_POINTER)
				continue;
