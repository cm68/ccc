...
/*
 * ld hl,X / push hl -> push X.  The Z280 pushes an immediate or a
 * memory word straight off, so materialising the operand into HL
 * first is a wasted instruction.  Same four bytes, one instruction
 * fewer, and HL survives.  Both spellings collapse: "ld hl,n" is
 * "push n" (IM) and "ld hl,(n)" is "push (n)" (DA).
 */
int
r_pushimm(void)
{
	...
	if (win[0].kind != L_INSN || !starts(0, "ld hl,"))
		return 0;
	/* only the immediate form: "ld hl,(X)" is a load, and the (sp+d)
	 * and (ix+d) register addresses have no push form at all */
	if (win[0].key[6] == '(')
		return 0;
	j1 = nextsig(0);
	if (j1 < 0 || win[j1].kind != L_INSN ||
...
applyrules(void)
{
	...
	case 'l':
		...
		case 'h':
			if (r_outi())	/* ld hl,n */
				return 1;
			return r_pushimm();	/* ld hl,n; push hl -> push n */
		case 'd':
...
report(void)
{
	...
	fprintf(stderr, "peep: exx %ld  m1cmp %ld  ccall %ld  cret %ld"
		"  jpnext %ld  hlarg %ld  noframe %ld  pushimm %ld"
		"  pool %ld = %ld bytes\n",
		n_exx, n_m1cmp, n_ccall, n_cret, n_jpnext, n_hlarg,
		n_noframe, n_pushimm, poolmerged, saved);
}
